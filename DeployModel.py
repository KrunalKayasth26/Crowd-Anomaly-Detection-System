...
# Load YOLO model
weights_path = "D:\AnomalyEventDetection-master\yolov4-p5.weights"
config_path = "D:\AnomalyEventDetection-master\yolo.cfg"
net = cv2.dnn.readNetFromDarknet(config_path, weights_path)
ln = net.getLayerNames()
ln = [ln[i[0] - 1] for i in net.getUnconnectedOutLayers()]

# Process video frames
cap = cv2.VideoCapture('D:\AnomalyEventDetection-master\videos for testing\fight.mp4')
while cap.isOpened():
    ret, frame = cap.read()
    if not ret:
        break
    ...
    # For example, check if the number of people exceeds a certain threshold
...
def RealTimeDetection(model, threshold, serve_type='real-time', vid_path=None, verbose=True):
    ...
    while(cap.isOpened()):
        ...
        if ret==True:
            esults = detect_people(frame, net, ln)
...
if __name__=='__main__':
    ...
    model_path = r'D:\AnomalyEventDetection-master\model.h5'
    
    #vid_path = './AvenueDataset/testing_videos/05.avi' #5,9
    vid_path = r'D:\AnomalyEventDetection-master\videos for testing\fight.mp4'
    
    frames_ext='.tif'
    frames_dir='Datasets/UCSDped2/Test'
    
    npy_file='.\Avenue_Dataset\tester.npy'
